...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Llamar al backend para pausar el contenedor Docker
    ...
    if (backendUrl) {
      try {
        ...
        await axios.post(`${backendUrl}/api/suite/pause`, {
        ...
        console.log(`Container ${name_service} stopped successfully`);
      } catch (dockerError) {
        console.error('Docker stop failed:', dockerError.message);
        ...
        return res.status(500).json({
          error: 'Error al pausar el contenedor',
    ...
    return res.status(200).json({
      success: true,
      message: `Instancia ${name_service} pausada exitosamente`
    });
