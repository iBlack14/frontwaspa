  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-transparent p-6 sm:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white dark:bg-[#1e293b] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Reglas Totales</div>
          <div className="text-2xl font-bold text-slate-800 dark:text-white">{rules.length}</div>
        </div>
        <div className="bg-white dark:bg-[#1e293b] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="text-emerald-600 dark:text-emerald-400 text-xs font-medium uppercase tracking-wider mb-1">Activas</div>
          <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{rules.filter(r => r.isActive).length}</div>
        </div>
        <div className="bg-white dark:bg-[#1e293b] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="text-slate-500 dark:text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Instancia</div>
          <div className="text-lg font-bold text-slate-800 dark:text-white truncate">
            {instances.find(i => i.documentId === selectedInstance)?.name || 'Seleccionar'}
          </div>
        </div>
        <div className="bg-white dark:bg-[#1e293b] p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="text-blue-500 dark:text-blue-400 text-xs font-medium uppercase tracking-wider mb-1">Estado</div>
          <div className={`text-lg font-bold ${chatbotActive ? 'text-emerald-500' : 'text-slate-400'}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {activeTab === 'rules' && (
          <>
            ...
            <div className={`bg-white dark:bg-[#1e293b] border ${editingRuleId ? 'border-indigo-500 ring-1 ring-indigo-500' : 'border-slate-100 dark:border-slate-800'} rounded-3xl p-6 shadow-sm transition-all`}>
              <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                {editingRuleId ? <PencilSquareIcon className="w-5 h-5 text-indigo-500" /> : <PlusIcon className="w-5 h-5" />}
                {editingRuleId ? 'Editar Regla' : 'Agregar Nueva Regla'}
              </h3>

              <div className="space-y-4">
                {/* ... (inputs for trigger and response) ... */}
                ...
                <div className="flex gap-3">
                  {editingRuleId && (
                    <button
                      ...
                      className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 py-3 px-4 rounded-xl hover:bg-slate-200 transition-colors font-medium"
                    >
                      Cancelar
                    </button>
                  )}
                  <button
                    type="button"
                    onClick={addRule}
                    className={`flex-1 ${editingRuleId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gradient-to-r from-emerald-500 to-teal-600'} text-white py-3 px-4 rounded-xl hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 font-medium flex items-center justify-center gap-2`}
                  >
                    {editingRuleId ? <CheckCircleIcon className="w-5 h-5" /> : <PlusIcon className="w-5 h-5" />}
            ...
            <div className="bg-white dark:bg-[#1e293b] border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 cursor-pointer" onClick={() => startEditing(rule)}>
                    {/* ... (rule content) ... */}
                  </div>
